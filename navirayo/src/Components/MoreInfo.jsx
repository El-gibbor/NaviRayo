...
function MoreInfo({ hide, id }) {
    ...
    return (
        <div className="fixed top-0 bottom-0 right-0 left-0 m-auto w-full h-full z-40">
            {/* full screen image */}
            <div
                className={`fixed top-0 left-0 w-full h-full z-50 flex flex-col items-center justify-center ${showImageFull ? "visible " : "invisible "
                    }`}
            >
                <div
                    onClick={() => setShowImageFull(false)}
                    className={`absolute top-0 left-0 bg-black/10 w-full h-full backdrop-blur-sm transition-all ease-out ${showImageFull ? "opacity-100 " : "opacity-0 "
                        } `}
                ></div>
                <img
                    src={image}
                    className={` max-h-[70%] h-[220px] brightness-[110%] min-w-[300px] max-w-[80%] object-cover shadow rounded-xl z-10 select-none transition-all ease-in ${showImageFull ? "opacity-100 " : "opacity-0 scale-95  "
                        }`}
                />
            </div>
            <div
                onClick={hideNotificationPopup}
                className={`w-full h-full fixed top-0 left-0 bg-stone-600/10 z-40 transition-all ease-in-out duration-300 ${animate ? "opacity-100" : "opacity-0"
                    } `}
            ></div>
            <div
                className={`absolute right-0 bg-white dark:bg-body-color-dark shadow-2xl text-dark-text dark:text-light-text w-full max-w-[500px] h-full z-40 p-4 flex flex-col transition-all ease-in-out duration-200  ${animate
