...
function MoreInfoTickets({ hide, id }) {
  ...
  const openedJourney = useMemo(
    () => ProvinceJourney.find((journey) => journey.id === id),
    [id]
  );
      ...
      >
        ...
        {load ? (
          ...
          <div className="h-full w-full max-w-[500px] flex flex-col gap-5 max-md:px-3 px-7 pt-8 pb-14 max-md:pb-32 overflow-y-auto">
            ...
            <Link to={`/purchaseticket/${openedJourney.id}`} className="min-h-[40px] w-full px-2 bg-main-color text-white font-semibold rounded-full text-sm flex items-center justify-center gap-1 cursor-pointer transition active:scale-95 ">
...
export default MoreInfoTickets;
